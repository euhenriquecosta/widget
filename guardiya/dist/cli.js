...
async function setupBoilerplate() {
    ...
    createFile(join(process.cwd(), "app/api/logout/route.ts"), `
...
`);
    ...
    createFile(join(process.cwd(), "app/dashboard/page.tsx"), `
  ...
  `);
    //logout_button
    createFile(join(process.cwd(), "components/LogoutButton.tsx"), `
...
`);
    //register/page.tsx
    createFile(join(process.cwd(), "app/register/page.tsx"), `
...
`);
    // login/page.tsx
    createFile(join(process.cwd(), "app/login/page.tsx"), `"use client";
...
export default function LoginPage() {
  ...
  return (
    ...
    // lib/auth.ts
    createFile(join(process.cwd(), "lib/auth.ts"), `import { createAuth } from "guardiya/server";
...
export const auth = createAuth({
  ...
  findUserByEmail: async (email: string) => {
    ...
    createFile(join(process.cwd(), "app/dashboard/layout.tsx"), `
...
`);
    ...
    createFile(join(process.cwd(), "app/api/login/route.ts"), `import { auth } from "@/lib/auth";
...
export async function POST(req: Request) {
  try {
    ...
    // lib/prisma.ts
    createFile(join(process.cwd(), "lib/prisma.ts"), `import { PrismaClient } from "@prisma/client";
...
function setupPostgresDockerCompose() {
    createFile(join(process.cwd(), "docker-compose.yml"), `version: "3.9"
